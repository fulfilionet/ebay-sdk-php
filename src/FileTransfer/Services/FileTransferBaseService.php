 ...
 */
class FileTransferBaseService extends \FulfilioNet\eBaySDK\Services\BaseService
{
    /**
    ...
    public static function getConfigDefinitions()
    {
        ...
        return $definitions + [
            'apiVersion' => [
                'valid' => ['string'],
                'default' => \FulfilioNet\eBaySDK\FileTransfer\Services\FileTransferService::API_VERSION
            ],
            'authToken' => [
                'valid' => ['string'],
                'required' => true
     ...
     */
    protected function callOperationAsync($name, \FulfilioNet\eBaySDK\Types\BaseType $request, $responseClass)
    {
        /**
        ...
        if ($name === 'uploadFile' && $request->hasAttachment()) {
            ...
            if (!isset($request->fileAttachment)) {
                $request->fileAttachment = new \FulfilioNet\eBaySDK\FileTransfer\Types\FileAttachment();
            }

            if (!isset($request->fileAttachment->Data)) {
                $request->fileAttachment->Data = new \FulfilioNet\eBaySDK\FileTransfer\Types\Data([
                    'xopInclude' => new \FulfilioNet\eBaySDK\FileTransfer\Types\XopInclude([
