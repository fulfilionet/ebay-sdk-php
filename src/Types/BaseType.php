...
use \FulfilioNet\eBaySDK\Types;
use \FulfilioNet\eBaySDK\Exceptions;
use \FulfilioNet\eBaySDK\JmesPath\Env;
use \FulfilioNet\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \FulfilioNet\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \FulfilioNet\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \FulfilioNet\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'FulfilioNet\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \FulfilioNet\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\FulfilioNet\eBaySDK\Types\BaseType', false)) {
