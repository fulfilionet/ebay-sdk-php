...
* SDK now ignores properties in the JSON response that would normally trigger an FulfilioNet\eBaySDK\Exceptions\UnknownPropertyException exception.
...
* Removed constant FulfilioNet\eBaySDK\Marketing\Enuns\InventoryCriterionEnum::C_INVALID.
* Removed property inventoryReferenceType from FulfilioNet\eBaySDK\Marketing\TypesInventoryItem class.
...
* Allow properties on `FulfilioNet\eBaySDK\Types\DecimalType` objects to be assigned both `integer` and `double` values.
...
* The protected method `\FulfilioNet\eBaySDK\Services::BaseService::callOperation` has been removed. Code that called this method will now need to call `callOperationAsync(...)->wait()` instead.
...
* The class `\FulfilioNet\eBaySDK\Handler` is now `\FulfilioNet\eBaySDK\HttpHandler`.
...
* Some properties where incorrectly declared as being `FulfilioNet\eBaySDK\ResolutionCaseManagement\Types\Integer` instead of `integer`.
