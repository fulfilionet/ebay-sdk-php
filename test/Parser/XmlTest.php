<?php
namespace FulfilioNet\eBaySDK\Parser\Test;

use FulfilioNet\eBaySDK\Parser\XmlParser;

class XmlTest extends \PHPUnit_Framework_TestCase
{
    public function testCanParseXml()
    {
        $xmlParser = new XmlParser('\FulfilioNet\eBaySDK\Test\Mocks\ComplexClass');
        $xml = file_get_contents(__DIR__.'/../Mocks/Response.xml');
        $xml = preg_replace('/[\n\r]/', '', $xml);
        $xml = preg_replace('/>\s+/', '>', $xml);
        $obj = $xmlParser->parse($xml);

        $this->assertInstanceOf('\FulfilioNet\eBaySDK\Test\Mocks\ComplexClass', $obj);
        ...
        $this->assertEquals('bar', $obj->strings[1]);
        $this->assertInstanceOf('\FulfilioNet\eBaySDK\Test\Mocks\SimpleClass', $obj->simpleClasses[0]);
        $this->assertEquals(888, $obj->simpleClasses[0]->integer);
        $this->assertInstanceOf('\FulfilioNet\eBaySDK\Test\Mocks\SimpleClass', $obj->simpleClasses[1]);
